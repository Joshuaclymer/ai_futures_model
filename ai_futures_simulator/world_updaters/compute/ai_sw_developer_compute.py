...
class AISoftwareDeveloperComputeUpdater(WorldUpdater):
    ...
    def _get_training_compute_growth_rate(self, nation_id: str, current_time: float) -> float:
        ...
        After the slowdown_year, the rate is reduced to post_slowdown_training_compute_growth_rate.
        """
        compute_params = self.params.compute
        ...
        if nation_id == NamedNations.USA:
            # US uses us_frontier_project_compute_annual_growth_rate
            ...
            if hasattr(compute_params, 'USComputeParameters') and compute_params.USComputeParameters:
                us_params = compute_params.USComputeParameters
                if hasattr(us_params, 'us_frontier_project_compute_annual_growth_rate'):
                    annual_growth_rate = us_params.us_frontier_project_compute_annual_growth_rate
                if hasattr(us_params, 'slowdown_year'):
                    slowdown_year = us_params.slowdown_year
                if hasattr(us_params, 'post_slowdown_training_compute_growth_rate'):
                    post_slowdown_rate = us_params.post_slowdown_training_compute_growth_rate
        elif nation_id == NamedNations.PRC:
    ...
    def set_metric_attributes(self, t: Tensor, world: World) -> World:
        """
        Update AI software developer compute from nation compute.

        For each developer:
        1. Get their nation's compute stock
        2. Apply fraction parameter to get developer's total compute
        3. Update operating_compute list
        4. Trigger __post_init__ to recompute allocation metrics

        NOTE: When software R&D is enabled, we ONLY update training_compute_growth_rate,
        NOT the compute allocation metrics. This preserves the CSV-initialized compute
        values which are needed for consistency with the old progress model.
        """
        # Check if software R&D updates are enabled
        skip_compute_update = (
            self.params.software_r_and_d is not None and
            self.params.software_r_and_d.update_software_progress
        )
        ...
        for dev_id, dev in world.ai_software_developers.items():
            ...
            # Update compute allocation only if software R&D is NOT updating progress
            # When software R&D is enabled, we preserve the CSV-initialized compute values
            if not skip_compute_update:
                # Get nation's functional compute (not limited by datacenter capacity)
                # AI software developers get a fraction of functional compute, not operating
                nation_compute = nation.compute_stock.functional_tpp_h100e
                if isinstance(nation_compute, Tensor):
                    nation_compute = nation_compute.item()

                # Get fraction of nation compute for this developer
                fraction = self._get_fraction_of_nation_compute(dev, nation_id)

                # Compute developer's total compute
                developer_compute = nation_compute * fraction

                # Get watts per H100e from nation's compute stock
                watts_per_h100e = 700.0  # Default H100 power
                if nation.compute_stock and nation.compute_stock.watts_per_h100e:
                    w = nation.compute_stock.watts_per_h100e
                    if isinstance(w, Tensor):
                        watts_per_h100e = w.item()
                    else:
                        watts_per_h100e = float(w)

                # Update developer's operating compute
                new_compute = Compute(
                ...
                # Update the operating_compute list (replace first entry or add)
                if dev.operating_compute:
                    dev.operating_compute[0] = new_compute
                else:
                    dev.operating_compute.append(new_compute)

                # Recompute allocation metrics
                # This updates ai_r_and_d_inference_compute_tpp_h100e, etc.
                total_compute = sum(c.functional_tpp_h100e for c in dev.operating_compute)
            ...
            # Always update training compute growth rate based on nation's compute growth
            # Pass current time to handle slowdown logic
            current_time = t.item() if isinstance(t, Tensor) else float(t)
            training_compute_growth_rate = self._get_training_compute_growth_rate(nation_id, current_time)
            dev._set_frozen_field('training_compute_growth_rate', training_compute_growth_rate)
