...
def compute_metrics_loop(
    ...
    training_compute_growth_rate = np.interp(times, data.time, data.training_compute_growth_rate)
    ...
    # ========== STEP 8: Compute training compute via trapezoidal integration ==========
    _t0 = time.perf_counter()
    training_compute = np.zeros(n_points)
    if n_points > 1:
        dt = np.diff(times)
        # Trapezoidal rule: avg of adjacent growth rates times dt
        avg_growth_rates = (training_compute_growth_rate[:-1] + training_compute_growth_rate[1:]) / 2.0
    ...
    # ========== STEP 12: Compute labor contributions and multipliers ==========
    _t0 = time.perf_counter()
    ...
    # ========== STEP 13: Compute progress_rates (overall rates) ==========
    _t0 = time.perf_counter()
    ...
    total_profiled = (_dt_interp + _dt_automation + _dt_taste + _dt_coding_labor +
                     ...
                     _dt_human_only + _dt_labor_contrib + _dt_progress_rate)
