...
import { PdfLineChart } from './PdfLineChart';
...
export interface MilestoneCdfPoint {
  year: number;
  cdf: number;
}
...
export interface MilestoneDistributionData {
  ...
  cdf?: { [key: string]: MilestoneCdfPoint[] };
}
...
const MilestoneDistributionChart = memo(({ data, className, title = 'Milestone Probability Density', height = 400, showCdf = false }: MilestoneDistributionChartProps) => {
  const { chartData, lines } = useMemo(() => {
    if (showCdf && data.cdf) {
      // Use CDF data
      const allYears = new Set<number>();

      // Collect all unique years from CDF data
      for (const milestoneName of Object.keys(data.cdf)) {
        for (const point of data.cdf[milestoneName]) {
          allYears.add(point.year);
        }
      }

      // Create a data point for each year with all milestone CDF values
      const sortedYears = Array.from(allYears).sort((a, b) => a - b);
      const combinedData = sortedYears.map(year => {
        const dataPoint: DataPoint = { x: year };

        for (const milestoneName of Object.keys(data.cdf!)) {
          const point = data.cdf![milestoneName].find(p => p.year === year);
          dataPoint[milestoneName] = point?.cdf ?? null;
        }
      ...
      const lineConfigs = Object.keys(data.cdf).map(milestoneName => ({
    ...
    // PDF mode: Combine all milestone data into a single dataset
    const allYears = new Set<number>();
  ...
  const yDomain = useMemo<[number, number]>(() => {
    // CDF mode uses 0-1 range with padding for label visibility
    if (showCdf) {
      return [-0.08, 1.05];
    }
  ...
  }, [chartData, data.milestones, showCdf]);

  const cumulativeProbabilityLookup = useMemo(() => {
    const lookup: Record<string, Map<number, number>> = {};

    for (const milestoneName of Object.keys(data.milestones)) {
      // Use empirical CDF data if available
      if (data.cdf && data.cdf[milestoneName]) {
        const cdfPoints = data.cdf[milestoneName];
        const probabilityMap = new Map<number, number>();
        for (const point of cdfPoints) {
          probabilityMap.set(point.year, point.cdf);
        }
        lookup[milestoneName] = probabilityMap;
        continue;
      }

      // Fallback to computing CDF from PDF (legacy behavior)
