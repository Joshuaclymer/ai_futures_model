...
from typing import Tuple, Optional, Any, Union, NamedTuple
...
class Parameters:
    ...
    def __post_init__(self):
        ...
        # Sanitize time horizon parameter - coerce to float first
        try:
            self.ac_time_horizon_minutes = _coerce_float_scalar(self.ac_time_horizon_minutes, "ac_time_horizon_minutes")
        except (TypeError, ValueError):
            self.ac_time_horizon_minutes = float('nan')
        ...
        # Set default progress_at_aa if not provided
        if self.progress_at_aa is None:
            self.progress_at_aa = 100.0  # Default upper anchor for automation
            logger.info(f"progress_at_aa was None, setting default to {self.progress_at_aa}")
...
def compute_software_progress_rate(research_stock: float, research_effort: float, 
    ...
    if r_software < 1e-8:
        logger.warning(f"Invalid r_software: {r_software}, must be >= 1e-8")
        return 0.0
...
class ProgressModel:
    ...
    def compute_progress_trajectory(self, time_range: list[float], initial_progress: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Add the upper anchor - use progress_at_aa if defined, otherwise use a default high value
        upper_anchor_progress = self.params.progress_at_aa if self.params.progress_at_aa is not None else 100.0
        automation_anchors[upper_anchor_progress] = self.params.automation_fraction_at_coding_automation_anchor
