...
**Goal:** Make the local model produce statistically equivalent outputs to the reference model for metrics like:
...
| What | Path |
|------|------|
| **Local model source** | `ai_futures_simulator/` |
...
**Note:** Uses direct mode (no HTTP server needed). Runs simulations in-process for speed.
