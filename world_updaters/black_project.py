    ...
    def contribute_state_derivatives(self, t: Tensor, world: World) -> StateDerivative:
        ...
        for project_id, project in world.black_projects.items():
            ...
            if project.datacenters is not None:
                ...
                # Only grow if after construction start and below max capacity
                if current_time >= dc.construction_start_year:
                    ...
                    if current_capacity < max_concealed and dc.construction_rate_gw_per_year > 0:
                        # Growth rate in log space: d(log(C))/dt = rate / C
                        # This gives linear growth in C: dC/dt = rate
                        # But we need to prevent it from exceeding max
                        headroom = max_concealed - current_capacity
                        effective_rate = min(dc.construction_rate_gw_per_year, headroom)

                        # Prevent division by zero for very small capacity
                        if current_capacity > 1e-10:
                            ...
                            # This will quickly bring capacity to a reasonable level
            ...
            if project.compute_stock is not None:
                ...
                hazard_rate = stock.current_hazard_rate()
...
def initialize_black_project(
    ...
    compute_stock = BlackComputeStock(
    ...
    datacenters = BlackDatacenterState(
