...
class SoftwareRAndD(WorldUpdater):
    ...
    def set_metric_attributes(
        ...
        for dev_id, dev in world.ai_software_developers.items():
            ...
            # Compute coding labor (uses optimal_ces or simple_ces based on coding_labor_mode)
            coding_labor = self._compute_coding_labor(progress, human_labor, inference_compute)
            dev.ai_software_progress.coding_labor = torch.tensor(coding_labor)
            dev.ai_software_progress.serial_coding_labor = torch.tensor(coding_labor)  # Same as coding_labor for simple mode
            ...
            human_only_coding = (human_labor ** params.parallel_penalty) * params.coding_labor_normalization
            ai_mult = coding_labor / human_only_coding if human_only_coding > 0 else 1.0
